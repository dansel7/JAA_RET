...
function confirmacion(id) {
         var answer = confirm("Esta seguro que desea eliminar este lider?")
        if (answer){
         window.location.href="lideres.php?eliminar_lider=true&id_lider="+id;
         }
         else{
...
<div id="cuerpo">
	...
	?>
              <form action="lideres.php" id="formulario" method="post" class="form-horizontal">
...
<div class="span8">
 <div>
    ...
    <table id="lista_grupos" align="center" cellpadding="10" cellspacing="0">
        ...
        $consulta="SELECT usuarios.id_usuario, usuarios.nombres, usuarios.apellidos, usuarios.correo, usuarios.password, lideres.activo, grupo.nombre as grupo, grupo.id_grupo, usuarios.telefono, usuarios.edad FROM lideres INNER JOIN usuarios ON usuarios.id_usuario=lideres.id_lider INNER JOIN grupo ON grupo.id_grupo=lideres.id_grupo";
		$res=mysql_query($consulta);
		$nombreh="";
		$casos_totales=0;
		$id_grupo=0;
		while($row=mysql_fetch_array($res)){
		        $id_lider=$row['id_usuario'];
				...
				echo "
					<tr class='tr_cuerpo'";?>onclick="location.href='<?php //echo $paso; ?>'"<?php echo ">	
						...
						<td style='text-align: center;'><a href='lideres.php?mod=true&id=$id_lider'><img src='../diseno/img/next.png' /></a></td>
				
				<td style='text-align: center;'><a href=\"javascript:confirmacion($id_lider)\"><img src='../diseno/img/close_16.png' /></a></td></tr> ";
							}
	?>
	<tr><td></td></tr>
	<tr><td></td></tr>
 ...
 if(mysql_query($consulta_new)){
	...
	while($row_id=mysql_fetch_array($res_id)){
 	...
 	if(mysql_query($consulta_new2)){
		...
		echo("<META HTTP-EQUIV='Refresh' CONTENT='0; URL=lideres.php'>");
	}
 }
...
if(isset($_REQUEST['eliminar_lider'])){
  $id_lider = $_GET['id_lider'];
  $consulta_eliminar = "DELETE FROM lideres WHERE id_lider=$id_lider";
  if(mysql_query($consulta_eliminar)){
  	  $consulta_eliminar2 = "DELETE FROM usuarios WHERE id_usuario=$id_lider";
