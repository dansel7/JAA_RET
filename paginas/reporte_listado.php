...
header("Content-Disposition: attachment; filename=reporte1_JAA2013.xls");
...
$consulta_grupo = "SELECT id_grupo, nombre FROM grupo WHERE id_grupo=(SELECT id_grupo FROM inscripcion WHERE id_hermano=$id_hermano)";
$res_grupo=mysql_query($consulta_grupo);
while($row_grupo=mysql_fetch_array($res_grupo)){
	$id_grupo=$row_grupo['id_grupo'];
	$grupo=$row_grupo['nombre'];
	if($id_grupo==1){
		$grupo="No asignado";	
	}
}	            
